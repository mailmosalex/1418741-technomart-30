// Pop-up окно карты формы обратной связи
...
window.addEventListener("keydown", function (evt) {
  27 == evt.keyCode && modalForm.classList.contains("modal-show") && modalForm.classList.remove("modal-show")
});
...
window.addEventListener("keydown", function (evt) {
  27 == evt.keyCode && modalMap.classList.contains("modal-show") && modalMap.classList.remove("modal-show")
});
...
function openService(evt, serviceName) {
  ...
  for (i = 0; i < tablinks.length; i++) {
      tablinks[i].className = tablinks[i].className.replace(" service-btn-active", "");
  }
